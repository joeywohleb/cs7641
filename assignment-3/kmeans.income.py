...
cols = ['AGE', 'WORK_CLASS', 'EDUCATION_NUM',
...
removedCols = ['CLASS_LABEL', 'EDUCATION', 'WORK_CLASS',
               'EDUCATION_NUM',  'MARITAL_STATUS', 'OCCUPATION', 'RELATIONSHIP', 'RACE', 'SEX', 'NATIVE_COUNTRY']
...
g = sns.PairGrid(data_train, hue="CLASS_LABEL", palette=sns.color_palette(
    "cubehelix", 3), vars=['AGE', 'CAPITAL_GAIN', 'CAPITAL_LOSS', 'HOURS_WORKED_PER_WEEK'])
g.map(plt.scatter)
...
visualizer.show(outpath="charts/income.k-means.KElbowVisualizer.png")
visualizer.poof()
...
visualizer.fit(X_train)        # Fit the data to the visualizer
# Finalize and render the figure
visualizer.show(outpath="charts/income.k-means.InterclusterDistance.png")
visualizer.poof()
...
visualizer.fit(X_train)       # Fit the data to the visualizer
# Finalize and render the figure
visualizer.show(outpath="charts/income.k-means.SilhouetteVisualizer.png")
